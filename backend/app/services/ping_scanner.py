...
IP Ping 扫描服务 - 实现并发 Ping 扫描功能和扫描结果处
"""
import asyncio
import platform
import time
...
class PingScanner:
    ...
    def __init__(
        ...
        Args:
            ...
            ping_count: 每个 IP Ping 次数
            use_proxy: 是否使用 Ping 代理服务
            proxy_url: Ping 代理服务URL
            source_ip: IP 地址（用于指定从哪个网络接口发ping
        """
    ...
    def _extract_response_time(self, output: str) -> Optional[float]:
        ...
        try:
            if self.system == "windows":
                # Windows 格式: time=XXms 时间=XXms time<1ms
...
class ScanResultProcessor:
    ...
    async def process_scan_results(
        ...
        """
        处理扫描结果的完整流
        1. 更新 IP 地址last_seen is_online 字段
        2. 标识未注册的在线 IP
        3. 生成扫描报告
        4. 持久化扫描历史记
