...
def find_segment_for_ip(db: Session, ip_str: str) -> Optional[NetworkSegment]:
    ...
    Returns:
        NetworkSegment 对象，如果未找到则返回 None
    """
    try:
...
def transition_ip_status(
    ...
    if new_status == "used":
        ...
        ip_address.allocated_at = datetime.utcnow()
...
def calculate_network_range(network: str, prefix_length: int) -> Tuple[str, str, int]:
    ...
    Returns:
        (first_ip, last_ip, total_ips): 第一个可用 IP、最后一个可用 IP、总 IP 数
    """
...
def validate_cidr(cidr: str) -> Tuple[bool, Optional[str], Optional[Tuple[str, int]]]:
    ...
    Args:
        cidr: CIDR 格式字符串 (如 "192.168.1.0/24")
...
def calculate_segment_ip_range(network: str, prefix_length: int) -> dict:
    ...
    Returns:
        包含网段范围信息的字典:
        {
            ...
            'first_usable_ip': str,      # 第一个可用 IP
            'last_usable_ip': str,       # 最后一个可用 IP
            'total_ips': int,            # 总 IP 数（排除网络地址和广播地址）
            'netmask': str,              # 子网掩码
        }
    """
    try:
...
def calculate_segment_usage(db: Session, segment_id: int) -> dict:
    ...
    # 计算使用率 = (已用 IP + 保留 IP + 在线 IP) / 总 IP × 100%
    # 修改后的需求
    if total_ips > 0:
