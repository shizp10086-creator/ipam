...
- SoftDeleteMixin: 软删除（deleted_at 字段），支持回收站
...
class OptimisticLockMixin:
    ...
    每次更新时 version 自动 +1，如果提交的 version 与数据库不一致，
    ...
    def update_segment(db, segment_id, data, current_version):
        rows = db.query(NetworkSegment).filter(
            ...
            NetworkSegment.version == current_version  # 关键：版本校验
        ).update({
            **data,
...
class TenantMixin:
    ...
    所有业务数据都带 tenant_id，通过中间件自动注入查询条件，
    确保租户间数据完全隔离。
    """
    @declared_attr
    def tenant_id(cls):
