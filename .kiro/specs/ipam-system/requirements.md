...
18. THE IPAM_System SHALL 采用双入口单应用架构，同一 Vue 应用内根据用户角色提供两套独立的界面入口：
    - **管理员设计态**（/admin/*）：侧边栏展示低代码设计器（表单/流程/报表/仪表盘/大屏/PPT 设计器）、系统配置（配置中心/定时任务/通知渠道）、插件管理、权限管理、模板管理、数据模型配置、租户管理
    - **用户运行态**（/app/*）：侧边栏展示业务功能（IP 管理/设备资产/机房机架/网络拓扑/告警中心/工单协作/终端管理/报表查看/仪表盘查看/自助门户）
19. THE IPAM_System SHALL 登录后根据用户角色自动跳转至对应入口：Administrator 默认进入 /admin，Regular_User 和 ReadOnly_User 默认进入 /app
20. THE IPAM_System SHALL 允许管理员在设计态和运行态之间一键切换（界面右上角提供切换按钮），普通用户无法进入设计态
21. THE IPAM_System SHALL 两套入口共享底层组件库（表格/表单/图表/卡片等通用组件），但页面布局框架（侧边栏/顶栏/内容区）独立，设计态使用设计器专用布局（左侧组件面板 + 中央画布 + 右侧属性面板），运行态使用标准管理后台布局（左侧导航 + 顶栏 + 内容区）
22. THE IPAM_System SHALL 管理员在设计态完成设计后，支持"角色模拟预览"功能：管理员可选择模拟任意角色（普通用户/只读用户/指定部门用户），系统以该角色的权限和数据范围渲染 /app 下的真实页面效果，包括菜单过滤、数据权限过滤、按钮显隐，确保管理员能看到最终用户的真实体验
23. THE IPAM_System SHALL 角色模拟预览时在页面顶部显示醒目的提示条（如"当前正在模拟【普通用户-张三】的视角"），提示条提供"退出模拟"按钮一键返回管理员视角，模拟期间所有操作为只读模式，不会产生真实数据变更
