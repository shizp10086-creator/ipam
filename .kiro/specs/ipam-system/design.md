...
│                          接入层 (Access Layer)                               │
│  ┌──────────────────────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │    Vue 3 单应用 (SPA)    │  │移动端/小程│  │第三方 API│  │ Webhook  │   │
│  │ ┌──────────┬───────────┐ │  │序/钉钉   │  │客户端    │  │ 入站     │   │
│  │ │/admin/*  │ /app/*    │ │  │          │  │(Jenkins) │  │          │   │
│  │ │设计态    │ 运行态    │ │  │          │  │          │  │          │   │
│  │ └──────────┴───────────┘ │  │          │  │          │  │          │   │
...
系统前端采用**双入口单应用**架构，同一个 Vue 3 应用内根据用户角色提供两套完全独立的界面体验。

#### 架构总览

```
┌─────────────────────────────────────────────────────────────────────┐
│                    Vue 3 单应用 (Single SPA)                         │
│                                                                     │
│  ┌─ 共享层 ────────────────────────────────────────────────────┐   │
│  │  Pinia 状态管理 │ Vue Router │ Axios 请求层 │ i18n 国际化   │   │
│  │  Element Plus │ ECharts │ AntV G6 │ 主题引擎 │ 权限指令     │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─ 通用组件库 ────────────────────────────────────────────────┐   │
│  │  IpamTable（高级表格：分页/排序/筛选/列自定义/导出）        │   │
│  │  IpamForm（动态表单：校验/联动/异步校验）                   │   │
│  │  IpamChart（图表封装：柱状图/折线图/饼图/仪表盘）           │   │
│  │  IpamSearch（全局搜索：智能意图识别）                       │   │
│  │  IpamTree（树形组件：网段嵌套/机房层级/分组管理）           │   │
│  │  IpamUpload（文件上传：Excel/图片/附件）                    │   │
│  │  IpamNotification（通知组件：告警弹窗/消息中心）            │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  ┌─────────────────────┐    ┌──────────────────────────────────┐   │
│  │  管理员设计态        │    │  用户运行态                      │   │
...
/login                          # 统一登录页
/admin/                         # 管理员设计态根路由
  /admin/dashboard              # 管理后台首页（系统健康度/插件状态/设计器入口）
  /admin/designer/form          # 表单设计器
  /admin/designer/workflow      # 流程设计器
  /admin/designer/report        # 报表设计器
  /admin/designer/dashboard     # 仪表盘设计器
  /admin/designer/screen        # 大屏设计器
  /admin/designer/ppt           # PPT 设计器
  /admin/designer/page          # 页面布局设计器
  /admin/config/system          # 系统配置中心
  /admin/config/notification    # 通知渠道配置
  /admin/config/scheduler       # 定时任务管理
  /admin/plugins                # 插件管理
  /admin/permissions            # 权限管理（角色/用户/管理员层级）
  /admin/tenants                # 租户管理
  /admin/templates              # 模板库管理
  /admin/connectors             # 自定义连接器
  /admin/automation             # 自动化工作流
  /admin/data-models            # 数据模型配置
/app/                           # 用户运行态根路由
  /app/dashboard                # 运维仪表盘（用户看到的是管理员设计好的仪表盘）
  /app/ipam/segments            # 网段管理
  /app/ipam/ips                 # IP 地址管理
  /app/ipam/matrix/:segmentId   # IP 矩阵视图
  /app/ipam/scan                # Ping 扫描
  /app/assets/devices           # 设备资产
  /app/assets/terminals         # 终端管理
  /app/assets/inventory         # 资产盘点
  /app/dcim/datacenters         # 机房管理
  /app/dcim/racks               # 机架管理
  /app/dcim/topology            # 网络拓扑
  /app/alerts                   # 告警中心
  /app/tickets                  # 工单列表
  /app/tickets/:id              # 工单详情
  /app/reports                  # 报表中心（查看管理员设计的报表）
  /app/nac                      # 准入管理
  /app/screen/:id               # 大屏展示（全屏模式）
  /app/portal                   # 员工自助门户
  /app/ai                       # AI 助手
  /app/settings                 # 个人设置
```
...
```mermaid
graph LR
    subgraph 设计态["管理员设计态 /admin"]
        FD["表单设计器"] -->|发布| FF["表单定义 JSON"]
        WD["流程设计器"] -->|发布| WF["流程定义 JSON"]
        RD["报表设计器"] -->|发布| RF["报表定义 JSON"]
        DD["仪表盘设计器"] -->|发布| DF["仪表盘定义 JSON"]
        SD["大屏设计器"] -->|发布| SF["大屏定义 JSON"]
        PD["页面布局设计器"] -->|发布| PF["页面布局 JSON"]
    end

    subgraph 运行态["用户运行态 /app"]
        FF -->|渲染| FR["表单渲染引擎"]
        WF -->|执行| WR["流程执行引擎"]
        RF -->|渲染| RR["报表渲染引擎"]
        DF -->|渲染| DR["仪表盘渲染引擎"]
    ...
    subgraph 存储["后端存储"]
        DB["designer_configs 表<br/>JSON 存储设计器产物"]
    end
...
核心思想：**管理员在设计态创建和配置，普通用户在运行态使用和操作**。设计器产物以 JSON 格式存储在 designer_configs 表中，运行态的渲染引擎读取 JSON 动态渲染页面。

#### 权限与入口控制

| 角色 | 默认入口 | 可访问范围 | 切换能力 |
|------|---------|-----------|---------|
| 超级管理员 | /admin | /admin/* + /app/* | 可切换到运行态 + 角色模拟预览 |
| 租户管理员 | /admin | /admin/*（本租户范围）+ /app/* | 可切换到运行态 + 角色模拟预览 |
| 普通用户 | /app | /app/*（权限范围内） | 不可进入设计态 |
| 只读用户 | /app | /app/*（只读） | 不可进入设计态 |
| 员工 | /app/portal | /app/portal/*（自助门户） | 不可进入其他页面 |

#### 角色模拟预览

管理员在设计态完成设计后，可通过"角色模拟预览"功能验证最终用户体验：

```
┌─────────────────────────────────────────────────────────────┐
│ ⚠️ 当前正在模拟【普通用户 - 张三（研发部）】的视角  [退出模拟] │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌────┬──────────────────────────────────────────────────┐ │
│  │导航│  （以张三的权限渲染的 /app 页面）                │ │
...
实现方式：
- 管理员点击"模拟预览"时，前端临时切换到目标用户的权限上下文（不修改 JWT，仅前端状态切换）
- 后端提供 `GET /api/v1/admin/simulate/{user_id}/permissions` 接口，返回目标用户的完整权限树和数据范围
- 前端路由守卫和权限指令读取模拟权限上下文，渲染对应的菜单和按钮
- 模拟期间所有写操作（POST/PUT/DELETE）被前端拦截，弹窗提示"模拟模式下不可操作"
...
frontend/src/
├── layouts/                    # 布局框架
│   ├── AdminLayout.vue         # 设计态布局（组件面板+画布+属性面板）
│   ├── AppLayout.vue           # 运行态布局（侧边栏+顶栏+内容区）
│   ├── ScreenLayout.vue        # 大屏全屏布局
│   └── PortalLayout.vue        # 员工自助门户布局
├── views/
│   ├── admin/                  # 设计态页面
│   │   ├── designer/           # 各类设计器
│   │   ├── config/             # 系统配置
...
│   │   ├── reports/            # 报表查看
│   │   └── portal/             # 自助门户
│   └── common/                 # 共享页面（登录/404/个人设置）
├── components/                 # 通用组件库（两个入口共享）
│   ├── IpamTable/
│   ├── IpamForm/
│   ├── IpamChart/
│   └── ...
├── designer/                   # 设计器框架（仅设计态使用）
│   ├── core/                   # 设计器核心（拖拽引擎/画布/属性面板）
│   ├── components/             # 设计器组件面板
│   └── renderers/              # 运行时渲染引擎
├── stores/                     # Pinia 状态管理
├── api/                        # API 请求层
├── router/                     # 路由配置（admin + app 双入口）
├── utils/                      # 工具函数
└── styles/                     # 样式（含主题变量）
```
